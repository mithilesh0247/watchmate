from django.urls import path, include
from rest_framework.routers import DefaultRouter
# from watchlist_app.views import movie_list, movie_details
from watchlist_app.api.views import WatchListListAV, StreamPlatFormVS, ReviewDetail, ReviewCreate, WatchDetailAV, ReviewList
from rest_framework import routers
router = DefaultRouter()
router.register('stream', StreamPlatFormVS,
                basename='streamplatform')

urlpatterns = [
    path('list/', WatchListListAV.as_view(), name='movie-list'),
    path('<int:pk>/', WatchDetailAV.as_view(), name='movie-details'),
    path('', include(router.urls)),
    #path('stream/', StreamPlatformAV.as_view(), name='movie-list'),
    #path('stream/<int:pk>/review', StreamPlatformAV.as_view(), name='movie-list'),
    path('review/<int:pk>', ReviewDetail.as_view(), name='review-detail'),
    path('<int:pk>/reviews/', ReviewList.as_view(), name='review-list'),
    path('<int:pk>/review-create/',
         ReviewCreate.as_view(), name='review-create'),
    #path('review/<int:pk>', ReviewDetail.as_view(), name='review-detail'),

]
